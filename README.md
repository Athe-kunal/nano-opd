...
- **`TRAIN_BATCH_SIZE`** — Per-step training microbatch behavior (as wired into `train_opd.py`).
- **`LR`** — Learning rate.
...
- **`DISTILL_TOP_K`** — Top-k used when matching teacher distribution (semantics follow `train_opd.py`).
...
- **`STUDENT_CHUNK_SIZE`** / **`TEACHER_CHUNK_SIZE`** — Chunk sizes for long-sequence handling; `-1` typically means “don’t chunk” (see how `train_opd.py` interprets them).
