...
def _prepare_llama_tp_model(model: nn.Module, device_mesh: dist.DeviceMesh):
...
def prepare_tp_model(model: nn.Module, device_mesh: dist.DeviceMesh):
    ...
    assert isinstance(model, (LlamaForCausalLM, Qwen2ForCausalLM, Qwen3ForCausalLM)), (
        "Tensor parallelism supports only causal LM models (LlamaForCausalLM, Qwen2ForCausalLM, Qwen3ForCausalLM); "
        f"tp_critic / token-classification models are not supported (got {model.__class__.__name__})."
    )
    _prepare_llama_tp_model(model, device_mesh)