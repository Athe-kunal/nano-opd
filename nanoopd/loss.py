...
def compute_forward_kl_loss(
    ...
    if top_k > 0:
        ...
        # KL over top-k vocab: sum_v pi(v) * [log pi(v) - log q(v)]
        per_token_kl = (s_topk_probs * (t_topk_logprobs - s_topk_logprobs)).sum(dim=-1)  # [B, seq_len-1]

    else:
        # Full vocab KL
        per_token_kl = (student_probs * (teacher_logprobs - student_logprobs)).sum(dim=-1)  # [B, seq_len-1]
